...
export async function saveProductChanges(admin, { original, updated }) {
  ...
  if (productChanged) {
    const response = await admin.graphql(UPDATE_PRODUCT, {
      variables: {
        input: {
          ...
          title: original.title !== updated.title ? updated.title : undefined,
          ...
          vendor:
            original.vendor !== updated.vendor ? updated.vendor : undefined,
          ...
          status:
            original.status !== updated.status ? updated.status : undefined,
    ...
    throwErrors(data.data.productUpdate.userErrors);
  }

  const changedVariants = updated.variants.filter((variant, index) => {
  ...
  if (changedVariants.length) {
    const response = await admin.graphql(UPDATE_VARIANTS, {
      variables: {
    ...
    throwErrors(data.data.productVariantsBulkUpdate.userErrors);
  }
